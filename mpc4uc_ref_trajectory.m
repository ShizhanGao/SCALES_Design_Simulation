...
% for i = 1:10
    % err_q = (1 - x(1))^2;
    % err_w = x(5:7);
    % ubg = Wq * err_q + Ww * (err_w' * err_w);
    % sol = solver('x0', u0, 'lbx', lb, 'ubx', ub, lbg, 0, 'ubg', ubg, 'p', x);
...
figure('Color', [1 1 1]);
subplot(5,1,1)
plot(time, Zhist(1:4,:)'); legend('q0','q1','q2','q3'); grid on;
subplot(5,1,2)
plot(time, Zhist(5:7,:)'); legend('wx','wy','wz'); grid on;
subplot(5,1,3)
stairs(time, [Vhist,Vhist(:,end)]'); legend('ux','uy'); grid on;
subplot(5,1,4)
% plot(Jhist'); legend('J'); title('Cost'); grid on;


% subplot(5,1,5)
% 3D attitude visualization (quaternion -> euler for plotting)
...
for k = 1:SimSteps
    ...
    dist_torque = [0; 0; 0]; % [0.1; 0.1; 0.1]*1e-4 + 0.5e-4*randn(3,1); 
...
for i = 1:length(log_Time)
    ...
    if idx <= size(Zhist, 2)
        z_curr = Zhist(:, idx);
        err_norm(i) = norm(log_X(:, i) - z_curr);
    end
end
